...
- 将 **Answer span** 和 **CoT span** 两个状态的神经元状态相减，得到**干预向量**
- 得到遗忘方向（遗忘子空间）
...
作者发现，如果你对所有输入都沿着 unlearning subspace 去改，很容易把模型正常 reasoning 的能力一起伤害。
...
然后做和前面类似的差分 + SVD【用来从差分表示中提取主要的方向，从而构建 unlearning subspace（遗忘子空间）】。得到的就是“支撑正常 reasoning 的方向”。
...
$\tilde{h} = h + \lambda f(h)$
其中，\( h \) 是原始隐藏状态，\( f(h) \) 是引导方向，\( \lambda \) 是固定比例系数。但这种方法的问题是：

- **强度固定**：无法根据当前状态动态调整引导强度。
- **不稳定性**：不同推理任务可能需要不同强度的引导。

为了解决这些问题，论文提出了 **自适应校准引导（ACS）**，通过最优传输（Optimal Transport，OT）方法来计算每次引导的强度。（最优运输距离”通过 目标方向角度（tar角） 来表达。）


### ACS 核心步骤：

1. **计算目标方向**：ACS 计算当前状态与目标方向之间的 **地理距离**（geodesic distance），即：

\$$
\theta_{\text{tar}} = \arccos \langle h^\hat{}, \hat{z}^* \rangle
\$$

2. **调整引导强度**：根据当前状态和目标的距离，ACS 动态调整引导强度 \( \lambda \)，其公式为：

\$$
\lambda = \min \left( 1, \frac{\theta_{\text{tar}}}{\theta_{\text{dir}}} \right)
\$$

3. **正交补投影**：ACS 使用 **正交补投影**，确保引导仅影响与 **RRS** 正交的部分。
...
\$$
\tilde{h} = r \cdot \text{slerp}(h^\hat{}, v^\hat{}; \lambda)
\$$

这里，\( r \) 是 \( h \) 的模长，\( \text{slerp} \) 是球面线性插值操作。

通过这种方式，ACS 在 **推理时** 实现了有效的 **unlearning**，并最大限度保留了模型的推理能力。
